...
class LLMBlock(Block):
    def __init__(self, block_name, config_path, client, model_id, output_cols, model_prompt="{prompt}", **batch_kwargs) -> None:
        super().__init__(block_name)
        self.block_config = self._load_config(config_path)
        self.prompt_struct = """{system}\n{introduction}\n{principles}\n{examples}\n{generation}"""
    ...
    def _parse(self, generated_string) -> dict:
        matches = {}
        for start_tag, end_tag, output_col in zip(self.block_config["start_tags"], self.block_config["end_tags"], self.output_cols):
            if not start_tag and not end_tag: 
                matches[output_col] = generated_string.strip() if generated_string else None
            else:
                pattern = re.escape(start_tag) + r'(.*?)' + re.escape(end_tag)
                all_matches = re.findall(pattern, generated_string, re.DOTALL)
                matches[output_col] = [match.strip() for match in all_matches] if all_matches else None
